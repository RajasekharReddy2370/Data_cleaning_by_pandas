...
# constituency_name = "Manthini"
# Assuming your original DataFrame is named 'df'
df = pd.read_excel(r"/home/rajashekar/Documents/Telangana_daily/Failed_Mobile_Numbers.xlsx")
...
for i in range(0, len(df), 5000):
    chunk = df.iloc[i:i+5000].copy()  # Get a chunk of 50,000 rows
    dfs.append(chunk)  # Append the chunk to the list
    if i < len(df) - 5000:  # Ensure we don't add an empty row after the last chunk
...
new_df.to_excel("/home/rajashekar/Documents/Telangana_daily/Failed_Mobile_Numbers_With_5000_space.xlsx",index=False)
