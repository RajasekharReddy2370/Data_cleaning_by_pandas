...
folder1 = "/home/rajashekar/Desktop/All_states_data/Telangana/Beneficiary/Mobile"
folder2 = "/home/rajashekar/Desktop/All_states_data/Telangana/Voter/Mobile"
...
def read_excel(file_path):
    return pd.read_excel(file_path, usecols=["Mobile"])
...
for prefix, file1 in files1.items():
    if prefix in files2:
        ...
        combined_df = pd.concat([df1, df2], ignore_index=True).drop_duplicates(subset=["Mobile"])

        # Save the combined data to a new Excel file
        output_file = f"/home/rajashekar/Desktop/All_states_data/Telangana/Beneficiary_Voter_Mobile/{prefix}.xlsx"
        combined_df.to_excel(output_file, index=False)
        C = len(combined_df)
        concat_df = pd.concat([concat_df, combined_df], ignore_index=True)
        constituency_lengths_Name_Mobile[prefix] = C

        print(f"Combined file saved for prefix '{prefix}'",C)
constituency_df_n_m = pd.DataFrame(list(constituency_lengths_Name_Mobile.items()), columns=['Constituency', 'Length'])
constituency_df_n_m.to_excel('/home/rajashekar/Desktop/All_states_data/State_constituency_lengths/Mobile/Telangana_constituency_lengths_Mobile.xlsx', index=False)
df3 = pd.read_excel(r"/home/rajashekar/Desktop/All_states_data/Telangana/Voter/Mobile/Bodhan_Mobile.xlsx",usecols = ["Mobile"])
print("############################BODHAN",len(df3))
concat_df.drop_duplicates(subset=["Mobile"],inplace=True)
df4 = pd.concat([df3,concat_df],ignore_index=True)
df4.drop_duplicates(subset=["Mobile"],inplace = True)

print( "ToTal MOBILE_NUMBERS",len(df4))
